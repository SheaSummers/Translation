...
def get_loader(data, src_vocab, tgt_vocab, src_tokenizer, tgt_tokenizer, batch_size, src_lang, tgt_lang, max_length):
    def data_process(data_sample):
        src = torch.tensor([[src_bos] + [src_vocab[token] for token in src_tokenizer(data_sample[src_lang])] + [src_eos]])
        tgt = torch.tensor([[tgt_bos] + [tgt_vocab[token] for token in tgt_tokenizer(data_sample[tgt_lang])] + [tgt_eos]])


        src = nn.ConstantPad1d((0, max_length - src.shape[1]), src_pad)(src)



        tgt = nn.ConstantPad1d((0, max_length - tgt.shape[1]), tgt_pad)(tgt)
        src = src.squeeze(0)
        tgt = tgt.squeeze(0)
    ...
    prep_data = [data_process(row) for _,row in data.iterrows()]
